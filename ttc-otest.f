...
C*                                                                        *
C*  TTCLAM:                                                               *
C*  This version is a modified one from David Harrison's TTC program.     *
C*  Instead of using UTM coordinates for grids it uses Lambert Conical    *
C*  coordinates.  The subroutine TRA2 by Gunnar Thorbergsson, Orkustofnun *
C*  Iceland is used for the transformation from Lambert to Geographical   *
C*  coordinates.  Data input is also slightly different.                  *
C*          Sara Sayyadi,  Magnus T. Gudmundsson                     *
C**************************************************************************
C*
...
C
C
C*  Ny Útgáfa fyrir Lahey fortran 90.  Les inn ASCII £tg fu af surfer
C*  grid skr .
C*  Innskr  1:   ASCII grid-skr 
C*  Innskr  2:   Skr  meŒ Lambertshnitum og free-air gildum fr  forritinu
C*               FREEAIR eŒa TTCLAM.
C*  §tskr :      Skr    sama formi og fr  FREEAIR en forritiŒ hefur
C*               skrifaŒ inn ¡ einn af TTC d lkunum.
C*
C*  essi £tg fa vinnur   tveimur griddum, annarsvegar 200x200 m,
C*  hins vegar 1000x1000 m.
C*
C*  ForritiŒ notar eŒlismassa 900 kg/m3 b‘Œi fyrir ¡s og berg.
C*
C*  ¥ £tskr nni eru fj¢rir d lkar fyrir landslags+Bouguer leiŒr.
C*  (d lkar 5-8):
C*   D lkar 5 og 7 eru fyrir ¡s (efra yfirborŒ).
C*   D lkar 6 og 8 eru fyrir berg (neŒra yfirborŒ).
C*   D lkar 5 og 6 eru fyrir leiŒr. fr  0 til 6 km (200x200 m grid).
C*   D lkar 7 og 8  "    "     "    fr  6 til 50 km (1000x1000 m grid).
C*
C*  ATH:  forritiŒ vinnur   "r‚ttum" Lambertshnitum.  Ekki hnitum meŒ
C*        X-hnit m¡nust”lu.  Skr rnar eru •¢ meŒ -X til samr‘mis viŒ
C*        grid af j”klum og landi   jarŒeŒlisfr‘Œistofu RH.
C*
C*                              7.7. 1997 MTG
C*
C*      !!! Breyting gerŒ 20.1 1998:  St”Œvarnafn sett fremst ¡ l¡nu ¡
C*      •yngdargagnaskr .  AllstaŒar •ar sem l¡nun£mer 3000 (formatsetning
...
C 2020-06-14 Changed read from file to format 5000:
      READ(9,5000,END=9900)
     + STNA,XSTA,YSTA,ZSTA,FA,T1,T1sea,T2,T2sea,TCORR,ZDIFF
!3000  FORMAT(A6,2F8.0,1f7.1,7F7.5)
      !write (*,*)FA
5000  FORMAT(A6,2F8.0,1f7.1,F9.2,6F9.5)
       !WRITE(*,5000)STNA,XSTA,YSTA,ZSTA,FA,T1,T1sea,T2,T2sea,TCORR,ZDIFF
C
...
C
      ...
      ! ELSEIF (INDI.EQ.3) THEN
        ...
        !GYMIN=GYMAX-2*50000.
       ENDIF
c      WRITE (10,*)FXMIN,FXMAX,FYMIN,FYMAX
